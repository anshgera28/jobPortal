...
export const postJob = async (req, res) => {
    try {
        ...
        if(!title || !description || !location || !salary || !experience || !jobType || !position || !companyId) {
...
export const getAllJobs = async (req, res) => {
    try {
        ...
        return res.status(200).json({ message: "Jobs found successfully", success: true, jobs });
    }catch (error) {
        console.log(error);
        return res.status(500).json({ message: "Internal server error", success: false, error: error.message });
    }
